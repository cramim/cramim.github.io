...
(function () {
    ...
    v2.add({
        ...
        apply: function () {
            ...
            /* המיסוך חייב לרוץ אחרי כל סינון, לא רק בפתיחת הקטגוריה. */
            v2.after("filter", apply_mask);

            /*  ה-hook לבדו לא מספיק.
             *
             *  app.js:861 קושר את ה-handler לאובייקט הפונקציה app.filter כפי
             *  שהיה ברגע ה-init, ו-app.js:948 מריץ את app.start לפני שלב ה-apply
             *  של v2 — כלומר ה-hook מחליף את app.filter רק אחרי שהקישור כבר
             *  תפס את הגרסה המקורית. לחיצה על מעגל או תזמון קוראת לכן ל-filter
             *  הלא-עטוף, המיסוך לא רץ, ופעילויות מקטגוריות אחרות חוזרות למסך.
             *
             *  listener מואצל על ה-wrapper רץ כשהאירוע מבעבע מה-input, כלומר
             *  אחרי ה-handler הישיר — וכך המיסוך חוזר למסלול. apply_mask רק
             *  מסתיר, ולכן הרצה כפולה שלו לא מזיקה.
             *
             *  במובייל הקישור הזה מנוטרל (mobile.js:114) והסינון עובר דרך
             *  #bt_apply_filter שקורא app.filter() בשמו — שם ה-hook כן תופס. */
            $("#filter_box_wrapper").on("change", "input[type='checkbox']", apply_mask);
        },
...
/*  שורת הפילטרים כתגיות.
 *
 *  ב-app.css שורת פילטר היא white-space:nowrap בתוך wrapper עם overflow:auto,
 *  ולכן שורת "תזמון" — שש תוויות ארוכות — גולשת לגלילה אופקית. בנוסף הכותרת
 *  היא inline-block ברוחב טבעי, כך ש"מעגל השפעה:" ו"תזמון:" דוחפות את הפריט
 *  הראשון של כל שורה למקום אחר והשורות לא מיושרות זו מול זו.
 *
 *  התיקון כולו ב-v2.css: הכותרות מקבלות עמודה ברוחב קבוע, הפריטים נשברים
 *  לשורות במקום לגלול, והצ'קבוקס מוחלף בתגית לחיצה. אין שינוי ב-HTML וב-app.js
 *  — ה-handler הקיים ב-app.js:844 ממשיך להמיר לחיצה על הפריט ללחיצה על ה-input.
 *
 *  דסקטופ בלבד: במובייל הפילטרים הם פאנל אנכי משלו (mobile.css:709) ואין שם
 *  לא גלילה אופקית ולא בעיית יישור.
 */
v2.add({
    id:    "filter-pills",
    title: "שורת פילטרים כתגיות במקום צ'קבוקסים",
    on:    true,
    pages: ["desktop"],
 ...
 *
 *  רצועה ברוחב מלא בתחתית הכרטיס, בגוון עדין ועם כיתוב פשוט. הכיתוב כולל את
 *  שם השדה ("מעגל השפעה") ולא רק את הערך, ולכן הוא מובן בלי מקרא ובלי סמל
 *  שצריך לפענח.
 *
 *  הכרטיס כבר נושא circle="..." כמאפיין (app.js:229), ולכן אין כאן שום עבודת
 *  נתונים — רק רינדור.
 *
 *  הרצועה ממוקמת absolute בתחתית הכרטיס ולא בזרימה. בדסקטופ min-height של
 *  15.5em (app.css:417) כבר מותח את הכרטיס מעבר לתוכן שלו, ולכן הרצועה יושבת
 *  בשטח שכבר שמור ולא מוסיפה גובה כלל. במובייל אין min-height, ולכן שם היא
 *  כן עולה משהו — ראה padding-bottom למטה ב-v2.css.
 *
 *  הרמה נגזרת ב-JS ולא בסלקטור CSS על הערך העברי. ערך שישונה בגיליון יקבל
 *  רצועה אפורה עם הטקסט הנכון במקום להישבר בשקט — בדיוק התקלה שתוקנה
 *  בקטגוריות.
 */
(function () {
        ...
        render: function () {
            $("#activity_boxes_wrapper .activity_box").each(function () {
                ...
                $("<div>")
                    ...
                    .append($("<span>").addClass("v2_circle_ttl").text("מעגל השפעה: "))
